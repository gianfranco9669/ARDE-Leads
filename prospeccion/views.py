...
def aplicar_busqueda_global(qs, texto):
    ...
    if rubros_tecnicos:
        filtro |= Q(rubro__in=rubros_tecnicos) | Q(campanias__rubro__in=rubros_tecnicos)
    if estado:
        filtro |= Q(estado_comercial=estado)
...
def dashboard(request):
    contexto = {
        ...
        'busquedas': BusquedaProspectos.objects.order_by('-creado_el')[:6],
        'oportunidades': Prospecto.objects.exclude(estado_comercial='do_not_contact').order_by('-puntaje_total')[:8],
        'duplicados': Prospecto.objects.filter(es_posible_duplicado=True)[:6],
        'no_contactar': Prospecto.objects.filter(estado_comercial='do_not_contact')[:6],
    }
...
def nueva_busqueda(request):
    if request.method == 'POST':
        ...
        if formulario.is_valid():
            ...
            busqueda.estado = 'queued'
            busqueda.creado_por = request.user if request.user.is_authenticated else None
            busqueda.solicitudes_estimadas = max(1, (busqueda.max_resultados + 19) // 20)
            busqueda.save()
...
def _filtros_activos(request):
    etiquetas = []
    simples = {
        'sin_web': 'Sin web detectada',
        'con_telefono': 'Con teléfono',
        'alta_prioridad': 'Alta prioridad',
        'duplicados': 'Duplicados posibles',
    }
    for clave, etiqueta in simples.items():
        if request.GET.get(clave):
            etiquetas.append({'clave': clave, 'etiqueta': etiqueta})
    estados = dict(Prospecto.ESTADOS_COMERCIALES)
    if request.GET.get('estado'):
        etiquetas.append({'clave': 'estado', 'etiqueta': f"Estado: {estados.get(request.GET['estado'], request.GET['estado'])}"})
    for clave, etiqueta in [('rubro', 'Rubro'), ('zona', 'Zona'), ('puntaje', 'Puntaje mínimo'), ('q', 'Búsqueda')]:
        valor = request.GET.get(clave, '').strip()
        if valor:
            etiquetas.append({'clave': clave, 'etiqueta': f'{etiqueta}: {valor}'})
    return etiquetas


def explorador(request):
    q = request.GET.get('q', '').strip()
    prospectos = aplicar_busqueda_global(Prospecto.objects.all().select_related('busqueda_origen').prefetch_related('campanias'), q)
    if request.GET.get('sin_web'):
        prospectos = prospectos.filter(tiene_web=False)
    if request.GET.get('con_telefono'):
        prospectos = prospectos.filter(tiene_telefono=True)
    if request.GET.get('alta_prioridad'):
        prospectos = prospectos.filter(puntaje_total__gte=75)
    if request.GET.get('duplicados'):
        prospectos = prospectos.filter(es_posible_duplicado=True)
    if request.GET.get('estado'):
        prospectos = prospectos.filter(estado_comercial=request.GET['estado'])
    if request.GET.get('rubro'):
        prospectos = prospectos.filter(rubro__icontains=request.GET['rubro'])
    if request.GET.get('zona'):
        prospectos = prospectos.filter(Q(ciudad__icontains=request.GET['zona']) | Q(zona__icontains=request.GET['zona']))
    if request.GET.get('puntaje'):
        prospectos = prospectos.filter(puntaje_total__gte=request.GET['puntaje'])
    contexto = {
        'prospectos': prospectos.order_by('-puntaje_total')[:200],
        'estados': Prospecto.ESTADOS_COMERCIALES,
        'q': q,
        'filtros': request.GET,
        'filtros_activos': _filtros_activos(request),
    }
    return render(request, 'prospeccion/explorador.html', contexto)
...
def cambiar_estado(request, pk, estado):
    ...
    if estado not in etiquetas:
        return HttpResponseBadRequest('Estado comercial inválido.')
    anterior = prospecto.get_estado_comercial_display()
    prospecto.estado_comercial = estado
    prospecto.save(update_fields=['estado_comercial', 'actualizado_el'])
    RegistroContacto.objects.create(prospecto=prospecto, canal='otro', direccion='saliente', estado='borrador', cuerpo_mensaje=f'Estado cambiado de {anterior} a {etiquetas[estado]}')
    messages.success(request, f'Estado actualizado a {etiquetas[estado]}.')
...
def accion_whatsapp(request, pk):
    ...
    generador = GeneradorLinkWhatsapp()
    accion = request.POST.get('accion', 'copiar')
    try:
...
def plantillas(request):
    ...
    if request.method == 'POST' and formulario.is_valid():
        ...
        return redirect('prospeccion:plantillas')
...
def desactivar_plantilla(request, pk):
    PlantillaMensaje.objects.filter(pk=pk).update(activa=False)
...
def campanias(request):
    ...
    if request.method == 'POST' and formulario.is_valid():
        ...
        partes = []
        if campania.rubro:
            candidatos = candidatos.filter(rubro__icontains=campania.rubro)
            partes.append(f"rubro similar a {RUBROS.get(campania.rubro, campania.rubro.replace('_', ' ').capitalize())}")
        if campania.zona:
            candidatos = candidatos.filter(Q(zona__icontains=campania.zona) | Q(ciudad__icontains=campania.zona))
            partes.append(f'zona {campania.zona}')
        campania.prospectos.add(*candidatos[:50])
        criterio = ', '.join(partes) or 'prospectos disponibles excluyendo No contactar'
        messages.success(request, f'Campaña creada. Se asociaron prospectos sugeridos por criterio: {criterio}.')
        return redirect('prospeccion:campanias')
...
def alternar_campania(request, pk):
    ...
    campania.estado = 'paused' if campania.estado == 'active' else 'active'
    campania.save(update_fields=['estado', 'actualizado_el'])
